...
/**
 *	Hebbian networks are one of the few autoassociative neural networks. An autoassociative
 *	network consists in to learn the same input pattern as output pattern. This networks just is able
 *	to learn binary patterns because its output function (signum).
 *	The Hebbian networks uses the Hebb's rule for trainning. The Hebb's rule is one of the most important
 *	training rules in unsupervised networks. Other algorithms like Kohonen uses this rule as base.
 *	@author kronenthaler
 */
public class Hebb extends NeuralNetwork{
	...
	/**
	 *	Constructor. Creates a Hebbian network with the equals number of inputs and outputs.
	 *  Set the decay constant to zero to eliminate it.
	 *	Alias of this(inputs, 0);
	...
	/**
	 *	Constructor. Creates a Hebbian network with the equals number of inputs and outputs.
	 *	Set the constant for decay <code>phi</code>. If phi = 0 the network don't forget anything, if phi = 1
	 *	the network just remember the las pattern. 
	...
	/**
	 *	Traing the network using the Hebb's rule with decay. The hebb's rule, consist on reinforce the right connections
	 *	if they produce a correct answer and inhibit the others. The decay term has an influence in how much affects the previous
	 *	knowledge to the reinforcement.
	...
	/**
	 *	Calculate the error of the networks using the Hamming distance instead the Euclidean.
	 *	Hamming distance is defined as the number of bits differents between to bits chains,e.g.
	 *	7=111 2=010 the hamming distance between H(7,2) = 2, the first and third bits differs.
