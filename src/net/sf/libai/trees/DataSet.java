...
/**
 * DataSet is a container compound by examples. Each example (or DataRecord) 
 * is a set of named attributes either discrete or continuous that represents 
 * a particular case of some type of class object.
 * This DataSet contain methods to manipulate those DataRecord and calculate
 * information gain, and entropy among the dataset.
 *<pre>
 * TODO:
...
public class DataSet {
	...
	/**
	 * Constructor.
	 * This constructor allows to create the DataRecords from unnamed attributes
	 * and setting the name positionally after the creation of the data set.
	 * Is a simplification of the next constructor.
	...
	/**
	 * Constructor.
	 * This constructor allows to create the DataRecords from named attributes.
	 * Also get some basic values used in the other methods.
	 * @param o		the index of the attribute considered as the output.
	 * @param data_	zero or more data records to build this data set.
	 */
	...
	/**
	 * Constructor.
	 * Create a data set from another data set.
	 * The new dataset will be the slice from <code>src[lo,hi)</code>
	...
	/**
	 * Sort this DataSet over a particular attribute <code>a</code>.
	 * As the attributes are comparables, the final sorting is decided by that function.
	...
	/**
	 * Calculate the information gain among the DataRecord's between <code>[lo,hi)</code> over the attribute
	 * <code>a</code>.
	 * Returns an array with the following values:{total entropy,amount of elements}
	 * @param lo	Lower bound (inclusive)
	 * @param hi	Upper bound (exclusive)
	 * @param a		Attribute to compare.
	 * @return		an array with the total entropy and the amount of elements involved (hi-lo).
	 */
	...
	/**
	 * Calculate the average information gain among the DataRecord's in <code>[lo,hi)</code>
	 * over the attribute <code>a</code>.
	 * The array returned depends of the nature of the attribute <code>a</code>. 
	 * If <code>a</code> is discrete, the array will be the returned by <code>infoAvgDiscrete</code>
	 * otherwise, will be the returned by <code>infoAvgContinuous</code>
	 * @param lo	Lower bound (inclusive)
	 * @param hi	Upper bound (exclusive)
	 * @param a		Attribute to compare.
	...
	/**
	 * Calculate the average information gain among the DataRecord's <code>[lo,hi)</code>
	 * over a discrete attribute <code>a</code>.
	 * Return an array with two positions: {average info gain, splitinfo gain}.
	 * The split information gain, is how much will be gained if you split the dataset over
	 * tha attribute <code>a</code>. Useful to calculate the Gain Ratio.
	 * @param lo	Lower bound (inclusive)
	 * @param hi	Upper bound (exclusive)
	 * @param a		Attribute to compare.
	 * @return an array with the average information gain and the split information gain.
	 */
	...
	/**
	 * Calculate the average information gain among the DataRecord's <code>[lo,hi)</code>
	 * over a continuos attribute <code>a</code>.
	 * Return an array with four positions:
	 * {maximum information gain possible,
	 *  maximun split information gain,
	 *  best value for the split,
	 *  index of the best value}.
	 * Over continuous attributes, the information gain is used in a binary way, lower than the best
	 * split and greater than the best split.
	 * To calculate the best split possible is necessary test all the possible splits in the
	 * continuous samples.
	 * @param lo	Lower bound (inclusive)
	 * @param hi	Upper bound (exclusive)
	 * @param a		Attribute to compare.
	...
	/**
	 * Calculate the absolute information gain among the elements in <code>[lo,hi)</code> over the
	 * attribute <code>a</code>.
	 * Returns an array with 4 or 6 positions (2+length(infoAvgDiscrete|infoAvgContinuous)):
	 * {absolute gain,
	 *  gain ratio,
	 *  values of infoAvgDiscrete|Continuos}
	 * @param lo	Lower bound (inclusive)
	 * @param hi	Upper bound (exclusive)
	 * @param a		Attribute to compare.
	...
	/** @return <code>true</code> if all the DataRecords have the same output, <code>false</code> otherwise. */
	...
	/**
	 * Check if all the DataRecord are equal in the DataSet.
	 * Return the most common output class.
	 * @return the most common output class if all are equal, <code>null</code> otherwise.
	 */
	...
	/**
	 * Split this data set into two new dataset where the propotion between the output classes is kept.
	 * The first dataset contains the <code>proportion</code> of the original data set, for instance,
	 * if the data set has 100 elements distributed between 2 classes, in a 60/40 proportion, this first
	 * set will contain (60*proportion + 40*proportion) elements and the second data set will contain the
	 * rest. This method is useful to generate trainning/test sets from one massive data set.
	 * @param proportion the percentaje of element to be keep of each class for the first data set.
